...
def _agt_remote_path_error(path: str, *, action: str, caller: str) -> str | None:
    if os.environ.get("HERMES_REMOTE_TARGET") != "vps":
        return None
    from agent.agt_gateway import agt_action_gateway, agt_block_error
    ...
    decision = agt_action_gateway(
        ...
        metadata={"remote_target": "vps", "path": path, "protected_local_target": path.startswith(("/etc", "/opt", "/root"))},
    )
    return agt_block_error(decision)
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        agt_error = _agt_remote_path_error(path, action="protected_target_path", caller="tools.file_tools.read_file_tool")
        if agt_error:
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    """
    agt_error = _agt_remote_path_error(path, action="protected_target_path", caller="tools.file_tools.write_file_tool")
    if agt_error:
