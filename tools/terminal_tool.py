...
def terminal_tool(
    ...
    try:
        ...
        remote_target = os.environ.get("HERMES_REMOTE_TARGET", "")
        if remote_target == "vps":
            ...
            decision = agt_action_gateway(
                ...
                metadata={
                    ...
                    "command": command,
                    "protected_local_target": any(part in command for part in ("/etc", "/opt", "/root")),
                },
            )
            error = agt_block_error(decision)
            if error:
                return json.dumps({"output": "", "exit_code": -1, "error": error, "status": "error"}, ensure_ascii=False)
