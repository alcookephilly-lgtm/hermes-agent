...
def delegate_task(
    ...
    async_reservation_token: Optional[str] = None
    if background:
        from tools.async_delegation import reserve_async_delegation_slot

        reservation = reserve_async_delegation_slot(
            max_async_children=_get_max_async_children(),
            metadata=metadata,
        )
        if reservation.get("status") != "reserved":
            return tool_error(reservation.get("error") or "Async delegation capacity reached.")
        async_reservation_token = reservation.get("reservation_token")
    ...
    try:
        for i, t in enumerate(task_list):
            ...
            effective_toolsets = t.get("toolsets", toolsets)
        ...
        if async_reservation_token:
            from tools.async_delegation import release_async_delegation_slot

            release_async_delegation_slot(async_reservation_token)
            async_reservation_token = None
        raise
    finally:
    ...
    if background:
        ...
        dispatch = dispatch_async_delegation_batch(
            ...
            max_async_children=_get_max_async_children(),
            reservation_token=async_reservation_token,
            metadata=metadata,
        )
