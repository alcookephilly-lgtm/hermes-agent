...
def test_single_completion_event_preserves_metadata():
    metadata = {"trace_id": "single-meta", "role_run_id": "rr-1"}

    def runner():
        return {"status": "completed", "summary": "ok"}

    res = ad.dispatch_async_delegation(
        goal="metadata single", context=None, toolsets=None, role="leaf", model="m",
        session_key="", runner=runner, max_async_children=3, metadata=metadata,
    )
    assert res["status"] == "dispatched"
...
def test_batch_completion_event_preserves_metadata():
    metadata = {"trace_id": "batch-meta", "role_run_id": "rr-2"}
...
def test_reservation_counts_as_active_and_reset_clears_it():
    reservation = ad.reserve_async_delegation_slot(max_async_children=1)
    assert reservation["status"] == "reserved"
    assert ad.active_count() == 1

    rejected = ad.reserve_async_delegation_slot(max_async_children=1)
