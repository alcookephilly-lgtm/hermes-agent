...
def test_janitor_worker_model_is_disabled_by_default_and_never_calls_recursive_hermes(monkeypatch):
    mm = load_plugin()
    ...
    assert "review_only_cleanup" in result["proposed_actions"]


def test_curator_worker_model_is_disabled_by_default_and_never_calls_recursive_hermes(monkeypatch):
    mm = load_plugin()
    provider = mm.MemoryMunchProvider()
    provider._session_id = ""  # avoid writing a test ledger event
    provider._scope_entity = "scope-a"
