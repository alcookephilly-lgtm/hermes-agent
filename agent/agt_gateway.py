    ...
    if decision == "allow" and "robot_hand_discovery_required" in policy_list:
        if meta.get("raw_discovery") is True:
            ...
            if "graphify-out" in target_text.lower() and "GRAPH_REPORT.md" in target_text:
                target_repo = str(meta.get("target_repo") or meta.get("repo_root") or meta.get("cwd") or "<target-repo>").rstrip("/")
                target_report = f"{target_repo}/graphify-out/GRAPH_REPORT.md"
                meta.setdefault("route_hint", "use smart-read robot hand for the target repo Graphify report before raw fallback")
                meta.setdefault("correct_tool_path", "mcp2cli @smart-read sc-read")
                meta.setdefault(
                    "retry_command_template",
                    f"mcp2cli '@smart-read' sc-read --file-path {target_report} --mode full --offset 1 --limit 80",
                )
