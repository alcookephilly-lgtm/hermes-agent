...
def set_friendly_tool_labels(enabled: bool) -> None:
    """Toggle friendly human-phrased tool labels."""
...
def build_tool_label(tool_name: str, args: dict, max_len: int | None = None) -> str | None:
    ...
    For built-in tools with a known verb, returns the verb optionally followed
    by the argument preview. For everything else, returns the raw preview.
    """
    if not _friendly_tool_labels:
