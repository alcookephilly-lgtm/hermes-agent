...
_NATIVE_UPDATE_SAFE_WORK_LEDGER_ESCALATION = "Graphify -> CodeGraph -> jcodemunch/smart-read -> Understand Anything"
_NATIVE_UPDATE_SAFE_WORK_STATES = ("dirty", "ahead", "diverged")
_NATIVE_UPDATE_SAFE_WORK_STATUSES = {"preserved", "update-safe"}


def _safe_work_truthy(value: Any) -> bool:
    if value in {None, False, "", "false", "False", "no", "NO", "0", 0, "unknown", "UNKNOWN"}:
        return False
    return bool(value)


def _safe_work_first(*maps: Mapping[str, Any], keys: tuple[str, ...]) -> Any:
    for mapping in maps:
        for key in keys:
...
def _read_native_update_safe_work_ledger(path_value: str | None) -> tuple[dict[str, Any] | None, str | None]:
    if not path_value:
        return None, "--safe-work-ledger PATH is required to classify preserved local work"
    path = Path(path_value).expanduser()
    if not path.is_file():
        return None, f"safe work ledger missing: {path}"
    try:
        text = path.read_text(encoding="utf-8")
    except OSError as exc:
        return None, f"safe work ledger read failed: {exc}"
    try:
        data = json.loads(text)
    except json.JSONDecodeError as exc:
        return None, f"safe work ledger must be JSON: {exc}"
    if not isinstance(data, dict):
        return None, "safe work ledger root must be a JSON object"
    return data, None


def _native_update_ledger_entries(ledger: Mapping[str, Any] | None) -> list[Mapping[str, Any]]:
    if not isinstance(ledger, Mapping):
        return []
    raw = ledger.get("entries") or ledger.get("local_work") or ledger.get("items")
...
def _native_update_entry_states(entry: Mapping[str, Any]) -> set[str]:
    values: list[str] = []
    raw = entry.get("states", entry.get("state", entry.get("local_work_states", [])))
    if isinstance(raw, str):
        values.append(raw)
    elif isinstance(raw, list):
        values.extend(str(item) for item in raw)
    for name in _NATIVE_UPDATE_SAFE_WORK_STATES:
        if entry.get(name) is True:
            values.append(name)
    return {value.strip().lower() for value in values if value.strip().lower() in _NATIVE_UPDATE_SAFE_WORK_STATES}


def _native_update_ledger_entry_for_state(ledger: Mapping[str, Any] | None, state_name: str) -> Mapping[str, Any]:
    for entry in _native_update_ledger_entries(ledger):
        if state_name in _native_update_entry_states(entry):
            return entry
    return {}


def _validate_native_update_ledger_state(
    ledger: Mapping[str, Any] | None,
    state_name: str,
) -> tuple[bool, str, Mapping[str, Any]]:
    entry = _native_update_ledger_entry_for_state(ledger, state_name)
    if not entry:
        return False, f"no safe work ledger entry for {state_name}", {}
    status = str(_safe_work_first(entry, ledger or {}, keys=("status", "ledger_status", "safe_work_ledger_status")) or "")
    if status not in _NATIVE_UPDATE_SAFE_WORK_STATUSES:
        return False, "status must be preserved or update-safe", entry
    if not _safe_work_truthy(_safe_work_first(entry, ledger or {}, keys=("restore_proof", "restore_path", "preservation_proof", "has_restore_proof"))):
        return False, "restore proof is required", entry
    if not _safe_work_truthy(_safe_work_first(entry, ledger or {}, keys=("test_proof", "test_result", "tests", "has_test_proof"))):
        return False, "test proof is required", entry
    if not _safe_work_truthy(_safe_work_first(entry, ledger or {}, keys=("decision_approval", "approval", "decision", "has_required_approval"))):
        return False, "decision/approval proof is required", entry
    if state_name == "diverged" and not _safe_work_truthy(
        _safe_work_first(entry, ledger or {}, keys=("divergence_plan", "rebase_plan", "cherry_pick_plan", "drop_plan", "plan"))
    ):
        return False, "diverged work requires rebase/cherry-pick/drop plan", entry
    return True, "safe work ledger proof accepted", entry


def _native_update_local_work_states(git_cmd: list[str], cwd: Path, branch: str) -> tuple[list[str], dict[str, Any]]:
    status = subprocess.run(
        git_cmd + ["status", "--porcelain"],
        cwd=cwd,
        capture_output=True,
    ...
    states: list[str] = []
    detail: dict[str, Any] = {"dirty_paths": []}
    if status.stdout.strip():
        states.append("dirty")
        detail["dirty_paths"] = [line[3:] if len(line) > 3 else line for line in status.stdout.splitlines()]

    upstream = f"origin/{branch}"
    local_count = _count_commits_between(git_cmd, cwd, upstream, "HEAD")
    remote_count = _count_commits_between(git_cmd, cwd, "HEAD", upstream)
    detail["local_ahead_count"] = local_count
    detail["remote_ahead_count"] = remote_count
...
def _print_native_update_safe_work_block(
    *,
    state_name: str,
    reason: str,
    branch: str,
    ...
    print()
    print(f"✗ Refusing native update: unknown or unproven local {state_name} work.")
    print("  This blocks before backup, gateway pause, lockfile discard, fetch, stash, checkout, pull, reset, or ZIP overwrite.")
    print(f"  branch: {branch}")
    print(f"  proof check: {reason}")
    dirty_paths = detail.get("dirty_paths") if isinstance(detail, Mapping) else None
    if dirty_paths:
        print("  dirty paths:")
        for path in list(dirty_paths)[:12]:
            print(f"    - {path}")
    print("  Classify/preserve the work, then retry with:")
    print("    hermes update --safe-work-ledger <PATH>")
    print(f"  If the meaning is unclear run: {_NATIVE_UPDATE_SAFE_WORK_LEDGER_ESCALATION}")


def _enforce_native_update_safe_work_ledger_gate(args, git_cmd: list[str], cwd: Path) -> dict[str, Any]:
    branch = _resolve_update_branch(args)
    states, detail = _native_update_local_work_states(git_cmd, cwd, branch)
    if not states:
        return {"states": [], "detail": detail, "ledger": None}

    ledger, ledger_error = _read_native_update_safe_work_ledger(getattr(args, "safe_work_ledger", None))
    metadata: dict[str, Any] = {
        "local_work_states": states,
        "safe_work_ledger": ledger or {},
        "safe_work_ledger_path": getattr(args, "safe_work_ledger", None),
    }
    from agent.agt_gateway import agt_action_gateway

    agt_decision = agt_action_gateway(
        action="native_update.local_work_gate",
        caller="hermes_cli.main._enforce_native_update_safe_work_ledger_gate",
        policies=("native_update_safe_work_ledger_required",),
        target=str(cwd),
        state={"branch": branch, "local_work_states": states, **detail},
        metadata=metadata,
    )

    for state_name in states:
        ok, reason, _entry = _validate_native_update_ledger_state(ledger, state_name)
        if ledger_error:
            ok = False
            reason = ledger_error
        if not ok or agt_decision.blocked:
            _print_native_update_safe_work_block(
                state_name=state_name,
                reason=agt_decision.reason if agt_decision.blocked else reason,
                branch=branch,
                detail=detail,
            )
            sys.exit(2)
    ...
    print(
        "  ⚠ Safe Work Ledger accepted for native-update local work: "
        + ", ".join(states)
    )
    return {"states": states, "detail": detail, "ledger": ledger}


def _enforce_native_update_zip_safe_work_ledger(args, cwd: Path) -> dict[str, Any]:
    """Guard ZIP overwrite installs where no git metadata exists to classify local work."""
    ledger, ledger_error = _read_native_update_safe_work_ledger(getattr(args, "safe_work_ledger", None))
    state_name = "dirty"
    detail: dict[str, Any] = {"dirty_paths": [str(cwd)], "zip_overwrite": True}
    metadata: dict[str, Any] = {
        "local_work_states": [state_name],
        "safe_work_ledger": ledger or {},
        "safe_work_ledger_path": getattr(args, "safe_work_ledger", None),
        "zip_overwrite": True,
    }
    ...
    agt_decision = agt_action_gateway(
        action="native_update.zip_overwrite_gate",
        caller="hermes_cli.main._enforce_native_update_zip_safe_work_ledger",
        policies=("native_update_safe_work_ledger_required",),
        target=str(cwd),
        state={"local_work_states": [state_name], **detail},
        metadata=metadata,
    )
    ok, reason, _entry = _validate_native_update_ledger_state(ledger, state_name)
    if ledger_error:
    ...
    if not ok or agt_decision.blocked:
        _print_native_update_safe_work_block(
            state_name=state_name,
            reason=agt_decision.reason if agt_decision.blocked else reason,
            branch="zip-overwrite",
            detail=detail,
        )
        sys.exit(2)
    print("  ⚠ Safe Work Ledger accepted for native-update ZIP overwrite path: dirty")
    return {"states": [state_name], "detail": detail, "ledger": ledger}
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    git_cmd_for_safe_work = ["git"]
    if sys.platform == "win32":
        git_cmd_for_safe_work = ["git", "-c", "windows.appendAtomically=false"]
    git_dir_for_safe_work = PROJECT_ROOT / ".git"
    if git_dir_for_safe_work.exists():
        _enforce_native_update_safe_work_ledger_gate(args, git_cmd_for_safe_work, PROJECT_ROOT)
    elif sys.platform == "win32":
        _enforce_native_update_zip_safe_work_ledger(args, PROJECT_ROOT)

    # Pre-update backup — runs after the Safe Work Ledger gate but before any
    # git/file mutation so users can roll back the exact pre-update state.
    ...
    try:
        ...
        try:
            from hermes_cli.agt_hardwire_manifest import guard_native_update_hardwires

            hardwire_guard = guard_native_update_hardwires(
                PROJECT_ROOT,
                f"origin/{branch}",
                command=f"git checkout/pull/reset origin/{branch}",
            ...
            print("✗ AGT hardwire manifest guard failed before update.")
            print(f"  {exc}")
            sys.exit(2)
